...
FiveBeansWorker.prototype.doNext = function()
{
	...
	self.client.reserve_with_timeout(self.timeout, function(err, jobID, payload)
	{
      ...
      self.emit('idle');
		...
		{
			self.client.stats_job(jobID, function(err, response)
			{
				if (err)
					self.emitWarning({ message: 'unable to receive stats', error: err });
				...
				try
                                ...
                                {
                                  self.emitWarning({ message: 'parsing job JSON', id: jobID, error: e });
                                  // errors out on `if !job` later
                                }

				try { self.emit('job.reserved', jobID); }
				catch (e) { return self.buryAndMoveOn(jobID); }

			        if (!job || !_.isObject(job)) {
					self.buryAndMoveOn(jobID);
				} else if (job instanceof Array) {
                                        job[1]._stats = response;
...
FiveBeansWorker.prototype.runJob = function(jobID, job)
{
	...
	{
		if (job._stats.reserves - job._stats.releases >= 2 && job._stats.kicks === 0)
		{
			if (handler.retryFailure) self.callHandler(handler, jobID, job, 'retryFailure');
			else self.buryAndMoveOn(jobID);
		}
		else
		{
			self.callHandler(handler, jobID, job, 'work');
		}
	}
};
...
FiveBeansWorker.prototype.callHandler = function callHandler(handler, jobID, jobdata, handlerFunction)
{
	if (handler.work.length === 3)
	{
		var patchedHandler =
		{
	        ...
	        FiveBeansWorker.prototype.doWork.call(this, patchedHandler, jobID, jobdata, handlerFunction);
	}
	else
	{
...
FiveBeansWorker.prototype.doWork = function doWork(handler, jobID, job, handlerFunction)
{
	var self = this;
	...
	{
		self.emit('job.timedOut', { id: jobID, handler: handler, job: job, type: handler.type, priority: job._stats ? job._stats.pri : undefined });
	};
	var timeoutTime = job._stats ? job._stats.ttr*1000 - 1000 : 60000;
	var timeout = setTimeout(timeoutFunc, timeoutTime);

	try
	{
		if (typeof job.payload === 'object')
		{
			job.payload._stats = job._stats;
			job.payload._stats.touch = function() {
                          clearTimeout(timeout);
                          timeout = setTimeout(timeoutFunc, timeoutTime);
                          self.client.touch(jobID, function(err) {
                            if (err) self.emit('error', err);
	                  });
                        };
		}

		handler[handlerFunction](job.payload, function(action, delay)
		{
			clearTimeout(timeout);
